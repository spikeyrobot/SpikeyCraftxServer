...
public class Client extends Thread
{
	...
	public void parse(Packet p)
	{
		try
		{
			...
			} else if (p.getType() == PacketTypes.PACKET_PLAYER_COMMAND)
			{
				LogManager.getInstance().info("Executing command /all (" + (String)p.getArgs() + ")");
				sendLocalCommand(((String)p.getArgs()).replaceFirst("/", ""));
	...
	public void sendLocalCommand(String s)
	{
		p.getServer().dispatchCommand(p.getServer().getConsoleSender(), s);
	}
